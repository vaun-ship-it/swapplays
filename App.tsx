...
function LoginScreen({ onEnter, authError }: { onEnter: (credentials: LoginCredentials) => Promise<string | void>; authError: string }) {
  ...
  const loginBottomPadding = isMobileWebLogin ? 0 : 24;
  const loginHorizontalSpace = isMobileWebLogin ? 76 : 20;
  const loginHeightBudget = isMobileWebLogin ? Math.max(320, height - 170) : height - 52;
...
const additionalPointLeaderNames = [
  "Nacho Average",
  "Ctrl Alt Defeat",
  "WiFi Bandit",
  "Couch Commando",
...
const additionalPointLeaders = additionalPointLeaderNames.map((name, index) => ({
  name,
  points: 184500 - index * 1850
}));

const leaderboardUsers = [...samplePointLeaders, ...additionalPointLeaders].map((user, index) => ({
